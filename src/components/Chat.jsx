...
const Chat = () => {
  ...
  const fetchChatHistory = async () => {
    try {
      const response = await axiosInstance.get("https://hosting-project-1.onrender.com/api/student/chat-messages");
      ...
      for (let message of chatMessages) {
        const senderResponse = await axiosInstance.get(`https://hosting-project-1.onrender.com/api/student/user?email=${message.sender}`);
  ...
  const handleSendChatMessage = async () => {
    try {
      const response = await axiosInstance.post("https://hosting-project-1.onrender.com/api/student/chat-messages", {
        message: chatMessage,
  ...
  const handleSendCommentMessage = async (index, commentMessage) => {
    try {
      const response = await axiosInstance.post(`https://hosting-project-1.onrender.com/api/student/chat-messages/${chatHistory[index]._id}/comments`, {
  ...
  const handleDeleteMessage = async (index) => {
    try {
      await axiosInstance.delete(`https://hosting-project-1.onrender.com/api/student/chat-messages/${chatHistory[index]._id}`);
      const updatedChatHistory = [...chatHistory];
  ...
  const handleSaveEdit = async (index) => {
    try {
      await axiosInstance.put(`https://hosting-project-1.onrender.com/api/student/chat-messages/${chatHistory[index]._id}`, { message: editMessage });
      const updatedChatHistory = [...chatHistory];
